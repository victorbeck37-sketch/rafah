...
import { generatePhpExportFiles } from './server/php_generator.js';
...
// In-memory active sessions: token -> { username, createdAt, csrfToken }
const activeSessions = new Map<string, { username: string; createdAt: number; csrfToken: string }>();
...
function requireAdminAuth(req: express.Request, res: express.Response, next: express.NextFunction) {
  const token = req.cookies['jardim_session'];
...
app.get('/api/csrf-token', (req, res) => {
  const token = req.cookies['jardim_session'];
  if (token && activeSessions.has(token)) {
    return res.json({ csrfToken: activeSessions.get(token)!.csrfToken });
  }
...
// 16. Generate & Export PHP 8.2+ Package for Shared Hosting
app.get('/api/admin/export-php', requireAdminAuth, (_req, res) => {
  try {
    const dir = generatePhpExportFiles();
    res.json({
      success: true,
      message: 'Pacote PHP 8.2+ gerado com sucesso para hospedagem compartilhada.',
      files: ['schema.sql', 'config.php', 'install.php', 'README.md'],
      path: dir
    });
  } catch (err: any) {
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`🌸 Servidor Nosso Jardim ao Entardecer rodando na porta ${PORT}`);
  });
}
